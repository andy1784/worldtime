#!/usr/bin/env python3
"""Shared generator for localized (translated) blog posts.

Localized posts live at blog/<slug>-<lang>.html  (e.g. convert-ist-to-gmt-ru.html).
vercel.json 301-redirects /<lang>/blog/<slug>.html -> /blog/<slug>-<lang>.html.

Each localized post mirrors the structure of the English original:
- <html lang="...">
- canonical -> the localized file
- hreflang alternates for x-default (en) + all 8 languages
- BlogPosting + BreadcrumbList + FAQPage schema (localized)
- article with breadcrumb, h1, localized meta, content, footer
- same JS converter logic
"""
import os, json
...
LANGS = ['en', 'ru', 'es', 'zh', 'ja', 'fr', 'de', 'uk', 'it']
...
def make_i18n_post(slug, lang, lang_code, title, h1, meta_desc, keywords,
                   breadcrumb, display_date, read_time, tags, content, faq_list):
    """lang: short code used in filename suffix (ru, es, zh, ja, fr, de, uk).
    lang_code: html lang attribute (same as lang, except may need region)."""
    filename = f'{slug}-{lang}.html'
    canonical = f'https://worldtimessync.com/blog/{filename}'
    en_url = f'https://worldtimessync.com/blog/{slug}.html'

    # hreflang alternates
    alts = []
    alts.append(f'    <link rel="alternate" hreflang="x-default" href="{en_url}">')
    for l in LANGS:
        if l == 'en':
            alts.append(f'    <link rel="alternate" hreflang="en" href="{en_url}">')
        else:
            alts.append(f'    <link rel="alternate" hreflang="{l}" href="https://worldtimessync.com/{l}/blog/{slug}.html">')
    alt_html = '\n'.join(alts)

    # If faq_list empty, extract FAQ from content HTML automatically
    if not faq_list:
        import re as _re
        faq_list = []
        for item in _re.findall(r'<div class="faq-item"><h3>(.*?)</h3><p>(.*?)</p></div>', content, _re.S):
            q = _re.sub(r'<[^>]+>', '', item[0]).strip()
            a = _re.sub(r'<[^>]+>', '', item[1]).strip()
            faq_list.append((q, a))
    faq_json = json.dumps(
        [{"@type": "Question", "name": q, "acceptedAnswer": {"@type": "Answer", "text": a}} for q, a in faq_list],
        ensure_ascii=False)

    with open(f'/home/kaliuser/worldtime/blog/{filename}', 'w', encoding='utf-8') as f:
        f.write('<!doctype html>\n')
        f.write(f'<html lang="{lang_code}">\n')
        f.write('<head>\n')
